...
**2026-05-09 — M1.2 (PR #16):** FTS5 contentless (**`replay_events_fts`**, `unicode61 remove_diacritics 2`); **`replay_fts_rebuild_signals`** + Trigger auf **`replay_events`**; **`ReplayFTSIndexer`** / **`ReplayFTSMetrics`**; **`SCHEMA_VERSION` 2** unter **`app/backend/ti_hub/db/`**.
...
**2026-05-10 — M1.10:** Vault **`r2-pull`** (`deploy/workers/r2-pull/pull.py`) löscht vor jedem **`litestream restore`** die lokale SQLite-Dateifamilie: Litestream überschreibt keine bestehende Zieldatei — ohne diesen Schritt bliebe der Vault-Spiegel nach der ersten erfolgreichen Restore-Iteration veraltet (symptomatisch: CI **`test_subsequent_changes_propagate`**).
